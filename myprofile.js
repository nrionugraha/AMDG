...
const CONTRACT_ABI = [
  ...
  "function tweets(uint) view returns (uint id, address author, string text, string imageUrl, uint timestamp, uint likeCount)",
...
async function loadMyTweets(userAddress) {
  ...
  try {
    ...
    for (let i = tweetCount; i > 0; i--) {
      ...
      // Hanya tampilkan tweet yang diposting oleh user saat ini
      if (tweet.author.toLowerCase() === userAddress.toLowerCase()) {
        // Konversi tweet.id ke Number
        const tweetId = tweet.id.toNumber();
        // Inisialisasi untuk menampung komentar terkait tweet ini
        ...
        // Ambil username tweet.author
        const tweetAuthorData = await contract.users(tweet.author);
        ...
        tweetCard.innerHTML = `
          <p>
            <strong>${tweetAuthorUsername}</strong> - ${new Date(tweet.timestamp.toNumber() * 1000).toLocaleString()}
          </p>
          <p>${tweet.text}</p>
